...
class QueriesControllerTest {
	...
	// query#RF
	/*
	 * @Test public void testGetTop10ImsiFailuresValidDate() { LocalDateTime
	 * startDate = LocalDateTime.of(2019, 4, 4, 6, 4, 2); LocalDateTime endDate =
	 * LocalDateTime.of(2024, 1, 2, 3, 4, 5); List<Object[]>
	 * testListTop10ImsiFailures = new ArrayList<>();
	 * //when(callFailureDAOMock.findTop10IMSIWithFailures(startDate,
	 * endDate)).thenReturn(testListTop10ImsiFailures); //ApiResponse<Object>
	 * response = queriesController.getTop10ImsiFailures(startDate, endDate);
	 * assertEquals(HttpStatus.OK.value(), response.getStatusCode());
	 * assertEquals("Success", response.getStatus()); }
	 * 
	 * @Test
	 * 
	 * void testGetTop10ImsiFailuresInValidDate() { LocalDateTime endDate =
	 * LocalDateTime.of(2015, 5, 5, 5, 5, 5); LocalDateTime startDate =
	 * LocalDateTime.of(2024, 4, 4, 4, 4, 4); ApiResponse<Object> response =
	 * queriesController.getTop10ImsiFailures(startDate, endDate);
	 * assertEquals(HttpStatus.BAD_REQUEST.value(), response.getStatusCode());
	 * assertEquals(null, response.getData());
	 * assertEquals("End date must be after start date",
	 * response.getError().getDetails()); assertEquals("Bad Date Range",
